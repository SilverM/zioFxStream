...
class Knnpredictor_2020_before():
    ...
    def run():
        ...
        currencyRelativeRate = 0.002 # 0.004 = 0.4%, 0.002
        ...
        # N_TRAIN = 900 #use 900 data points to train
        N_TRAIN = round(len(df)*0.3) ## use 30% of data to train
### note that the KNeighborsClassifier model repeats below, the top is just for runing on jupyter lab to see results, can remove also
        class MLTrainOnceStrategy(Strategy):
            price_delta = .1   # 0.04 = 0.4%

            def init(self):        
                # Init our model, a kNN classifier
                self.clf = KNeighborsClassifier(100)
            ...
            def next(self):
                ...
                elif forecast == 0 and (self.position.is_short or  self.position.is_long) :
                    self.position.close()
